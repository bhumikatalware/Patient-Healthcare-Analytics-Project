# Patient-Healthcare-Analytics-Project

Overview

This project is an end-to-end Data Analytics project focused on analyzing healthcare data using Python, SQL, and Power BI. The project covers the complete analytics workflow, including data loading, exploratory data analysis (EDA), data cleaning, SQL-based analysis, dashboard creation, reporting, and presentation preparation.

The main objective is to transform raw healthcare data into meaningful insights that help understand patient demographics, hospital visits, treatment costs, lab test trends, and healthcare performance indicators.

Dataset
The dataset contains healthcare-related information divided into multiple tables:

Patient Data – Patient demographics, age, gender, blood type, insurance provider, chronic conditions
Doctor Data – Doctor specialties, experience, and hospital details
Visit Data – Patient visit records, diagnoses, visit type, and follow-up information
Treatment Data – Treatment details, costs, outcomes, and status
Lab Test Data – Test names, results, abnormalities, and reference ranges
Tools & Technologies Used
Tool / Technology	Purpose
Python	Data Cleaning & EDA
Pandas & NumPy	Data Manipulation
Jupyter Notebook	Analysis Environment
PostgreSQL / MySQL / SQL Server	SQL Querying & Analysis
Power BI	Dashboard & Data Visualization
Gamma	Presentation (PPT) Creation
GitHub	Project Hosting & Portfolio
Project Workflow

1. Data Loading in Python
Imported datasets using pandas
Loaded CSV files into Jupyter Notebook
Checked dataset structure and data types

2. Exploratory Data Analysis (EDA)
Analyzed dataset statistics
Identified missing values and duplicates
Explored distributions and trends
Generated insights from healthcare records

3. Data Cleaning
Removed duplicate records
Handled missing values
Standardized column names
Corrected inconsistent data
Converted data types

4. SQL Analysis

Performed SQL analysis using PostgreSQL/MySQL/SQL Server to generate KPIs and business insights.

Key SQL Analysis
Total Patients, Visits, Treatments, and Lab Tests
Patient Demographics Analysis
Follow-Up Rate Analysis
Treatment Cost Analysis
Diagnosis & Visit Trends
Abnormal Lab Test Analysis
Doctor Workload Analysis

5. Power BI Dashboard

Created an interactive dashboard to visualize healthcare insights.
Dashboard Features
KPI Cards
Patient Demographics Visualization
Treatment Cost Analysis
Visit Trends
Lab Test Result Analysis
Interactive Filters & Slicers
Doctor Performance Insights

6. Report Creation
Prepared a detailed project report summarizing:
Dataset overview
EDA findings
SQL analysis
Dashboard insights
Business recommendations

Project Files
File Name	Description
Paitent_Healthcare.ipynb	Python EDA & Data Cleaning
Paitent_Project_3.sql	SQL Queries & Analysis
power bi project.pbix	Power BI Dashboard
README.md	Project Documentation
How to Run the Project
Python
Open Jupyter Notebook
Run the Paitent_Healthcare.ipynb file
Cleaned datasets will be generated
SQL
Create a database in PostgreSQL/MySQL/SQL Server
Import the cleaned datasets
Run the SQL queries from Paitent_Project_3.sql
Power BI
Open the .pbix file in Power BI Desktop
Refresh the data connection
Explore dashboard insights
Skills Demonstrated
Data Cleaning
Exploratory Data Analysis (EDA)
SQL Query Writing
KPI Development
Dashboard Development
Data Visualization
Business Insight Generation
Healthcare Data Analysis

Conclusion
This project demonstrates a complete data analytics workflow using Python, SQL, and Power BI. It highlights practical skills in data cleaning, SQL analysis, dashboard development, reporting, and presentation creation while generating meaningful healthcare insights.

Author
Bhumika Talware
Data Analytics Project Portfolio
